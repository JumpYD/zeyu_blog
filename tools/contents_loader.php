...
foreach ($draft_files as $draft)
{
	...
	{
		LogOpt::set('exception', '草稿原文不存在', 'article_id', $article_id, MySqlOpt::errno(), MySqlOpt::error());
		continue;
	}
	echo '是否加载该草稿到日志原文？ 《'.$article_info[0]['title'].'》(arctile_id:'.$article_id.') [y/N]';
	$sure = fgets(STDIN);
	...
	if ($ret == null)
	{
		LogOpt::set ('exception', 'article 更新失败', 'article_id', $article_id, MySqlOpt::errno(), MySqlOpt::error());
		return;
	}
